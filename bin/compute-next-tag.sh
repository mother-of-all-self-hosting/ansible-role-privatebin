...
#
# Tags look like `v<PrivateBin version>-<release>`, which is what this
# repository has always published (v1.7.8-0 ... v2.0.6-0):
#
# - if defaults/main.yml points at a PrivateBin version that has never been
#   released, the release counter restarts at 0 (`v2.0.7-0`)
# - otherwise the counter is incremented (`v2.0.7-1`), but only if something
...
# The commit-message approach this replaced also mistook Renovate's "Docker
# tag to v2" subject for a version and published a `v2-0` tag off it, while
# defaults/main.yml said 1.7.8 all along.
...
# Anchored on `privatebin_version:` so that neither `privatebin_version_alpine`
# - the Alpine flavour that the self-build git tag is built out of - nor
# `privatebin_container_image_tag`, which is derived from it, can be mistaken
# for it.
version="$(sed -nE 's|^privatebin_version:[[:space:]]*"?([^"[:space:]]+)"?.*$|\1|p' "$defaults_path" | head -n1)"

if [ -z "$version" ]; then
	echo >&2 "Could not determine the PrivateBin version from $defaults_path"
	exit 1
fi

# PrivateBin's own version is carried without a leading `v` (the `v` lives in
# the tags only), but tolerate one so that a future change of convention does
# not produce a doubled prefix.
